...
#
# Run inside a Husker VM:
#   husker exec <vm> -- sh /path/to/check-k3s-kernel.sh
#
...
check CONFIG_BRIDGE_NETFILTER "Bridge netfilter hooks"
...
check CONFIG_IP_VS_LC "IPVS least-connection"
...
check CONFIG_NET_UDP_TUNNEL "UDP tunnel support"
check CONFIG_NF_TABLES "nftables"
check CONFIG_NFT_NAT "nftables NAT"
check CONFIG_NFT_COMPAT "iptables-nft compatibility"
...
check CONFIG_CGROUP_BPF "cgroup v2 device policy"
...
check CONFIG_BPF_SYSCALL "BPF syscall"
check CONFIG_BPF_JIT "BPF JIT"
